...
""", {'MAVHEAD': get_mavhead(xml)})
    ...
    this.header = new ${MAVHEAD}.header(this._id, this.payload.length, mav.seq, mav.srcSystem, mav.srcComponent, incompat_flags, 0,);    
...
""" % (m.fmtstr, get_mavhead(xml), m.name.upper(), m.order_map, m.len_map,  m.array_len_map, m.crc_extra, m.name.upper()))
...
${MAVHEAD}.messages.${MNAME}.prototype.pack = function(mav) {
    ...
    var j = jspack.Pack(this.format, flattened);
...
def generate_tests_mavlink_class(outf, msgs, xml):
    ...
    for m in msgs:
        ...
        outf.write("   console.log('testing:%s'); \n" % (  m.name.lower() ) )
        ...
        for f in m.ordered_fieldnames:
            ...
            if _isarray and (fieldtype == 'char'):
                tdata = 'new Buffer.from('+m.test_data[idx]+').toString()';
            ...
            elif _isarray and ( (fieldtype == 'float') or (fieldtype == 'uint16_t') or (fieldtype == 'int16_t') or (fieldtype == 'double')  ):
                tdata = m.test_data[idx];
            # array of other things
            elif _isarray:
                tdata = 'new Buffer.from('+m.test_data[idx]+')';  
            elif _isfloat:
                    tdata = m.test_data[idx];
            elif _isnum:
                     tdata = m.test_data[idx];   
...
//${MAVPROCESSOR}Tests = function(logger, srcSystem, srcComponent) { }
""", {'MAVHEAD': get_mavhead(xml), 'MAVPROCESSOR': get_mavprocessor(xml), 'PROTOCOL_MARKER' : xml.protocol_marker})
def generate_tests_footer(outf, xml):
    t.write(outf, """
// Expose this code as a module
//module.exports = {${MAVHEAD}Tests, ${MAVPROCESSOR}Tests};
""", {'MAVHEAD': get_mavhead(xml), 'MAVPROCESSOR': get_mavprocessor(xml)})
